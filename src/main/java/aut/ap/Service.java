...
public class Service {
    ...
    public Email readEmailByCode(String code, String currentUserEmail) {
        return sessionFactory.fromTransaction(session -> {
            ...
            if (emails.size() == 1) {
                ...
                if (email.getSenderEmail().equals(currentUserEmail) || Arrays.asList(recipients).contains(currentUserEmail)) {
                    if (!email.isRead()) {
                        email.setRead(true);
                        session.merge(email);
                    }
    ...
    public Email replyToEmail(String code, String replyBody, String senderEmail) {
        return sessionFactory.fromTransaction(session -> {
            ...
            if (originalEmails.isEmpty()) {
                System.out.println("Error: Original email not found.");
                return null;
            }
    ...
    public Email forwardEmail(String Code, String newRecipients, String senderEmail) {
        return sessionFactory.fromTransaction(session -> {
            ...
            String newSubject = "[Fw] " + originalEmail.getSubject();
            String newBody = originalEmail.getBody();
